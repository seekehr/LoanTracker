...
router.post('/', async (req, res) => {
    const username = typeof req.headers["username"] === "string" ? req.headers["username"] : "Invalid.";
    const password = typeof req.headers["password"] === "string" ? req.headers["password"] : "Invalid.";
    const displayName = typeof req.headers["displayname"] === "string" ? req.headers["displayname"] : "Invalid.";
    const country = typeof req.headers["country"] === "string" ? req.headers["country"] : "Invalid.";
    let ip = req.verifiedIp;
    const errors = [];
    if (!username || username === "Invalid." || !SecurityGuard.verifyName(username))
        errors.push('Missing/invalid header: username');
    if (!password || password === "Invalid." || !SecurityGuard.verifyPassword(password))
        errors.push('Missing header/invalid: password');
    if (!displayName || displayName === "Invalid." || !SecurityGuard.verifyDisplayName(displayName))
        errors.push('Missing header/invalid: displayname');
    if (!country || country === "Invalid." || country.length > 2)
        errors.push('Missing header/invalid: country');
    if (ip === undefined || ip === INVALID_IP)
        errors.push('Could not determine IP address.');
    if (errors.length > 0) {
